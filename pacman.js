...
Pacman.User = function (game, map) {
    ...
    function move(ctx) {
        ...
        if ((isMidSquare(position.y) || isMidSquare(position.x)) &&
            ...
            for (var i=0; i<pill_list.length;i++){
                if (position.y == pill_list[i][0] * 10 && position.x == pill_list[i][1] * 10){
                    document.getElementById("scrolltext").innerHTML = pill_list[i][2]
    ...
    function draw(ctx) { 
        ...
        ctx.fillStyle = "white";  
        var font = "bold " + s/3 +"px serif";
        ctx.font = font;
        ctx.fillText(text, ((position.x/10) * s) + s / 2- s/8, ((position.y/10) * s) + s / 2-s/8);
    };
...
var PACMAN = (function () {
    ...
    var state        = WAITING,
        ...
        preStartTime    = Math.floor(Date.now() / 1000)-20;
    ...
    function dialog(text) {
        ctx.fillStyle = "#FFFF00";
        ctx.font      = "22px BDCartoonShoutRegular";
        var width = ctx.measureText(text).width,
            x     = ((map.width * map.blockSize) - width) / 2;        
        ctx.fillText(text, x, (map.height * 2) + 8);
    }

    function soundDisabled() {
        return localStorage["soundDisabled"] === "false";
    };
    ...
    function drawFooter() {
        ...
        for (var i = 0, len = user.getLives(); i < len; i++) {
            ...
            ctx.fill();
            ctx.fillStyle = "white";  
            var font = "bold " + map.blockSize / 4 +"px serif";
            ctx.font = font;
            var text = "M"
            ctx.fillText(text,  150 + (25 * i) + map.blockSize / 2 - map.blockSize / 8, (topLeft+1) + map.blockSize / 2 - map.blockSize / 8);
        }
    ...
    function mainDraw() { 
        ...
        ghostPos = [];
        ctx.drawImage(background,0,0,3364/3.2,1912/3); 
        map.drawPills(ctx);
    ...
    function readData(json){
        try{
            ...
            req.send(null);
            if (req.status == 200) {
                var geopoints = req.responseText.substring(1, req.responseText.length-1).split(',');
                lat = parseFloat(geopoints[0]);
                lon = parseFloat(geopoints[1]);
                group_name = geopoints[2]
                map_lat = Math.round(19 * (90 - lat) / 180)
                map_lon = Math.round(29 * (180 + lon) / 360)-2
                pill_list.push([map_lat, map_lon, group_name]);
            }
        }
        catch(err){
            console.log(err)
    ...
    function mainLoop() {
        ...
        if (state === PLAYING) {
            while (preStartTime != startTime){
                readData("http://127.0.0.1:7379/GET/"+startTime+".html");
                console.log(startTime);
                preStartTime = startTime;
            }

            for (var i=0; i<pill_list.length;i++){
                map.setBlock2(pill_list[i][0], pill_list[i][1], Pacman.PILL)
            }
            mainDraw();
    ...
    function init(wrapper, root) {
        ...
        var i, len, ghost,
            blockSize = wrapper.offsetWidth / 29,
            canvas    = document.createElement("canvas");
        
        canvas.setAttribute("width", (blockSize * 29) + "px");
        canvas.setAttribute("height", (blockSize * 19) + 80 + "px");
...
Pacman.MAP = [
    ...
    // [0, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 0],
    // [0, 2, 2, 2, 2, 2, 2, 2, 2, 3, 2, 2, 2, 2, 2, 2, 2, 2, 0],
    [0, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 0],
    [0, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 0],
    // [0, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 0],
    // [0, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 0],
